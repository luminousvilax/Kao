...
function SequenceSettingsMenu({ isEditing, toggleEdit, isCustom, onResetSequence, sequence, job, onImportSequence }) {
  ...
  return (
    <div className="settings-menu-container" ref={menuRef}>
      {isEditing ? (
        ...
        <>
          <button className="settings-btn" onClick={() => setIsOpen(!isOpen)} title="Sequence Options">
            <Icons.Settings />
          </button>
...
function InsertTrigger({ onInsert, isActive }) {
  return (
    <div className={`insert-trigger ${isActive ? 'active' : ''}`} onClick={onInsert} title="Insert Step Here">
  ...
  return (
    <div className="inline-add-row">
      <div className="inline-add-form">
        <select value={nodeId} onChange={handleNodeChange} title={selectedNode?.displayName}>
          {Object.entries(getGroupedNodes()).map(
            ([groupName, nodes]) =>
              nodes.length > 0 && (
      ...
      {error && (
        <div style={{ color: '#ef4444', fontSize: '0.8rem', marginLeft: '0.5rem', marginTop: '0.25rem' }}>{error}</div>
      )}
    </div>
  );
}
        ...
        <div className="priority-title-row">
          ...
          {!isEditing && (
            <label className="show-reached-toggle">
              <input type="checkbox" checked={showReached} onChange={(e) => setShowReached(e.target.checked)} />
      ...
      <div className="sequence-scroll">
        {(!sequence || sequence.length === 0) && !isEditing ? (
          ...
          sequence.map((step, idx) => {
            ...
            return <PriorityItem key={uniqueId} step={step} node={node} isDone={isDone} onComplete={onCompleteStep} />;
          })
        )}
      </div>
    </div>
